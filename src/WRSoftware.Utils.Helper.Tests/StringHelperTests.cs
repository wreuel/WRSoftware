using System;
using System.Collections.Generic;
using System.Linq;
using System.Runtime.InteropServices;
using Xunit;

namespace WRSoftware.Utils.Helper.Tests
{
    public class StringHelperTests
    {
        [Theory]
        [InlineData(8)]
        [InlineData(10)]
        [InlineData(20)]
        public void GenerateDummyPassword_ReturnsExpected(int expected)
        {
            var str = StringHelper.GenerateDummyPassword(expected);
            Assert.Equal(expected, str.Length);
        }

        [Theory]
        [InlineData("Error")]
        [InlineData("My Message")]
        public void ConvertStringToCollection_ReturnsExpected(string expected)
        {
            var list = StringHelper.ConvertStringToCollection(expected);

            Assert.Single(list);
            Assert.Equal(expected, list.FirstOrDefault());
            Assert.IsAssignableFrom<IEnumerable<string>>(list);

        }

        public static IEnumerable<object[]> Exceptions()
        {
            yield return new object[] { "Error", 2, new Exception("Error", new Exception("Inner Exception")) };
            yield return new object[] { "Error", 1, new Exception("Error") };
        }

        [Theory]
        [MemberData(nameof(Exceptions))]
        public void ConvertStringToCollectionException_ReturnsExpected(string strExpected, int sizeExpected, Exception value)
        {
            var list = StringHelper.ConvertStringToCollection(value);

            Assert.Equal(strExpected, list.FirstOrDefault());
            Assert.Equal(sizeExpected, list.Count());
            Assert.IsAssignableFrom<IEnumerable<string>>(list);

        }

        [Theory]
        [InlineData(null)]
        public void ConvertCollectionToStringEmpty_ReturnsExpected(IEnumerable<string> expected)
        {
            var result = StringHelper.ConvertCollectionToString(expected);
            Assert.Equal(string.Empty, result);
        }

        public static IEnumerable<object[]> ListString()
        {
            yield return new object[] { "teste<br />testes", new List<string>() { "teste", "testes" } };

        }

        [Theory]
        [MemberData(nameof(ListString))]
        public void ConvertCollectionToString__ReturnsExpected(string expected, IEnumerable<string> entry)
        {
            var result = StringHelper.ConvertCollectionToString(entry);
            Assert.Equal(expected, result);
        }


        [Theory]
        [InlineData("5925250", "59.252.50")]
        [InlineData("", "ABCDEFGHIJKLMNOPQRSTUVWXYZ!@#$%^&*()_+")]
        [InlineData("8485", "A84B85")]
        public void OnlyNumber_ReturnsExpected(string expected, string entry)
        {
            var result = StringHelper.OnlyNumber(entry);
            Assert.Equal(expected, result);
        }

        [Fact]
        public void GenerateRandomCode_ReturnsDefaultSize()
        {
            var result = StringHelper.GenerateRandomCode();
            Assert.Equal(6, result.Length);
        }


        [Theory]
        [InlineData(6)]
        [InlineData(0)]
        [InlineData(100)]
        public void GenerateRandomCode_ReturnsExpected(int expected)
        {
            var result = StringHelper.GenerateRandomCode(expected);
            Assert.Equal(expected, result.Length);
        }

        [Theory]
        [InlineData("http://www.example.com")]
        public void CheckIfStringIsAbsoluteUrl_ReturnsTrue(string url)
        {
            var result = StringHelper.CheckIfStringIsAbsoluteUrl(url);
            Assert.True(result);
        }

        [Theory]
        [InlineData("/xyz.html")]
        public void CheckIfStringIsAbsoluteUrl_ReturnsFalse(string url)
        {
            var result = StringHelper.CheckIfStringIsAbsoluteUrl(url);
            Assert.False(result);
        }


        [Theory]
        [InlineData("Error")]
        public void CheckIfStringIsValidBase64_ReturnFalse(string base64)
        {
            var result = StringHelper.CheckIfStringIsValidBase64(base64);
            Assert.False(result);
        }

        private const string image =
            "data:image/png;base64,";

        [Theory]
        [InlineData(image)]
        public void CheckIfStringIsValidBase64_ReturnTrue(string base64)
        {
            var result = StringHelper.CheckIfStringIsValidBase64(base64);
            Assert.True(result);
        }


        [Theory]
        [InlineData("png", image)]
        public void GetExtensionFromBase64String_Expected(string ext, string base64)
        {
            var result = StringHelper.GetExtensionFromBase64String(base64);
            Assert.Equal(ext, result);
        }


        [Theory]
        [InlineData("image/png", image)]
        public void GetContentTypeFromBase64String_Expected(string type, string base64)
        {
            var result = StringHelper.GetContentTypeFromBase64String(base64);
            Assert.Equal(type, result);
        }


        [Theory]
        [InlineData(image)]
        public void GetByteArrayFromBase64String_Expected(string base64)
        {
            var result = StringHelper.GetByteArrayFromBase64String(base64);

            Assert.IsType<byte[]>(result);
        }




    }
}

